...
The development deployment now serves the Vite app publicly, seeds fixtures idempotently, exposes capability-scoped realtime queries, and records live OpenAI ClaimEnvelope extraction plus Firecrawl search/scrape and a bounded durable manufacturer crawl. A sanitized live browser proof reached `VERIFIED_RECALL_UNSAFE_CHANNEL` from exact CPSC recall `25-459` and model `PZB02-E001`, exposed the independently recovered `recall@epoca.com` action, then used a version-bound approval to deliver through AgentMail to the visibly separate demo destination. Signed delivery webhooks and a bounded scheduled status reconciler update the Convex communication timeline. A real forwarded notice and reply still need an end-to-end smoke test.
