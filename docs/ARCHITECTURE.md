...
The development deployment now serves the Vite app publicly, seeds fixtures idempotently, exposes capability-scoped realtime queries, and records live OpenAI ClaimEnvelope extraction plus Firecrawl search/scrape and a bounded durable manufacturer crawl. A sanitized live browser proof reached `VERIFIED_RECALL_UNSAFE_CHANNEL` from exact CPSC recall `25-459` and model `PZB02-E001`, then exposed the independently recovered `recall@epoca.com` action. AgentMail webhook/send adapters are deployed with safe missing-secret behavior, but real AgentMail events are not claimed until its account credentials and inbox are configured.
