...
The production deployment serves the Vite app publicly, seeds fixtures idempotently, and has passed a fresh-browser smoke test plus a live OpenAI/Firecrawl deterministic-verdict proof. The development deployment records the complete AgentMail proof: a distinct forward created a case, a version-bound approval delivered to the visibly separate demo destination, and signed replies attached to the trusted thread. Production has its own enabled, inbox-scoped AgentMail webhook and secret; unsigned requests are rejected. Signed delivery webhooks and a bounded scheduled reconciler update the Convex communication timeline.
