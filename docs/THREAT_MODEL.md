...
| Secret leakage                      | Secrets only in Convex env; `.env*` ignored except names-only example; no raw payload logging                                            | Repository/diff scans pass; OpenAI live path inspected; AgentMail runtime audit blocked |
...
- OpenAI extraction has been exercised with a sanitized notice and persisted only after strict schema validation. AgentMail credentials are not configured, so real webhook delivery and sending have not yet been exercised; the missing credential aborts before approval consumption.
