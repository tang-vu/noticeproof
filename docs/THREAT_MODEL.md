...
| Malformed/hallucinated model output | Responses `parse` + strict Zod schema, exact-quote span validation, unique-quote offset repair, at most two attempts, no partial persistence | Implemented, adversarially tested, and exercised live                 |
...
| Cross-case subscription leak        | Public fixtures are explicitly allowlisted; private reads require capability; public demo/proof/crawl queries use least-privilege objects    | Implemented and tested                                                |
