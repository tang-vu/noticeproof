...
| 1   | in_progress | Thin real sponsor slice through inbound → extraction → evidence → reactive UI → approved controlled send  | AgentMail credentials                     | Convex + OpenAI + Firecrawl live; AgentMail event proof blocked |
...
- Real OpenAI and Firecrawl acceptance evidence is recorded on the development deployment; the full AgentMail acceptance gate still requires its server-side credentials and inbox.
